...
async function loadClubData() {
  try {
    const [clubRes, membersRes, matchRes] = await Promise.allSettled([
      fetchEA(`/clubs/overallStats?platform=${PLATFORM}&clubIds=${CLUB_ID}`),
      fetchEA(`/clubs/members?platform=${PLATFORM}&clubId=${CLUB_ID}`),
      fetchEA(`/clubs/matches?matchType=gameType9&platform=${PLATFORM}&clubIds=${CLUB_ID}`),
    ]);

    if (clubRes.status === 'fulfilled') applyClubStats(clubRes.value);
    else showApiNote('Club-Stats konnten nicht geladen werden.');

    if (membersRes.status === 'fulfilled') {
      buildSquad(membersRes.value);
      if (membersRes.value.members) buildTopScorers(membersRes.value.members);
    } else showSquadError();
  ...
  grid.innerHTML = sorted.map(p => {
    ...
    return `
      <div class="player-card${isCapt ? ' player-card--captain' : ''}" data-pos="${pos.group}">
        ...
        ${isCapt ? '<div class="captain-badge">Kapitän</div>' : ''}
...
function showSquadError() {
  document.getElementById('squadGrid').innerHTML = '<p class="no-data">Squad-Daten konnten nicht geladen werden.</p>';
}

function buildTopScorers(members) {
  const sorted = [...members].filter(p => (p.skGoals || 0) > 0).sort((a, b) => (b.skGoals || 0) - (a.skGoals || 0)).slice(0, 5);
  ...
  document.getElementById('topScorers').innerHTML = sorted.map((p, i) => `
    <div class="stats-bar-item">
      <div class="stats-bar-meta"><span>${p.name || p.proName}</span><span>${p.skGoals} Tore</span></div>
      <div class="stats-bar-track"><div class="stats-bar-fill" data-width="${Math.round((p.skGoals / max) * 95)}" style="background:linear-gradient(90deg,${colors[i]});width:0%"></div></div>
    </div>`).join('');

  setTimeout(() => { document.querySelectorAll('.stats-bar-fill').forEach(el => { el.style.width = el.dataset.width + '%'; }); }, 300);
}

function buildMatches(data) {
  const matches = Array.isArray(data) ? data : (data.matches || []);
  const list = document.getElementById('matchList');
  if (!matches.length) { list.innerHTML = '<p class="no-data">Keine Spiele gefunden.</p>'; return; }
  ...
  list.innerHTML = matches.slice(0, 8).map(m => {
    ...
    const date = m.timestamp ? new Date(m.timestamp * 1000).toLocaleDateString('de-DE', { day: 'numeric', month: 'short' }) : '–';
    ...
    return `
      <div class="match-item match-item--${result}">
        ...
        <div class="match-teams">
          ...
          <div class="match-score"><span>${ourGoals}</span><span class="match-score-sep">:</span><span>${oppGoals}</span></div>
...
function showMatchError() {
  document.getElementById('matchList').innerHTML = '<p class="no-data">Spielergebnisse konnten nicht geladen werden.</p>';
}
...
function initFilters() {
  document.querySelectorAll('.filter-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      document.querySelectorAll('.player-card').forEach(c => { c.classList.toggle('hidden', f !== 'all' && c.dataset.pos !== f); });
    });
  });
}
...
document.querySelectorAll('.about-card, .stat-card').forEach(el => { el.classList.add('reveal'); revealObserver.observe(el); });
...
if (joinForm) {
  joinForm.addEventListener('submit', (e) => {
    e.preventDefault();
    joinForm.style.opacity = '0'; joinForm.style.transform = 'scale(0.95)'; joinForm.style.transition = '0.3s ease';
