...
document.querySelectorAll('.about-card, .player-card, .stat-card, .match-item, .stats-bar-item').forEach(el => { el.classList.add('reveal'); revealObserver.observe(el); });

const heroObserver = new IntersectionObserver((entries) => {
  if (entries[0].isIntersecting) { document.querySelectorAll('.hero-stat__number').forEach(animateCounter); heroObserver.disconnect(); }
}, { threshold: 0.5 });
...
const barObserver = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    if (!entry.isIntersecting) return;
    entry.target.querySelectorAll('.stats-bar-fill').forEach(fill => { fill.style.width = fill.dataset.width + '%'; });
...
document.querySelectorAll('.filter-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    document.querySelectorAll('.player-card').forEach(card => { card.classList.toggle('hidden', filter !== 'all' && card.dataset.pos !== filter); });
  });
});
...
document.querySelectorAll('.tab-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    document.querySelectorAll('.match-list').forEach(list => { list.classList.toggle('hidden', list.id !== tab); });
  });
});
